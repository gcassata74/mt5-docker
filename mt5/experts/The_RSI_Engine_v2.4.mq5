...
#property version   "2.4" // v2.4: divergence → same-sign slopes → entry; exit only on price slope sign reversal
...
input bool   InpUse_Slope_Alignment_Exit = true;  // Exit when price slope reverses sign
...
int OnInit()
{
    ...
    PrintFormat("RSI Engine v2.4 initialized. BreakEven trigger: %d pts, Trailing trigger: %d pts",
...
void OnDeinit(const int reason)
{
    ...
    Print("RSI Engine v2.4 deinitialized. Reason: ", reason);
}
...
//+------------------------------------------------------------------+
//| Regression channel band SL: trails with slope, absorbs normal   |
//| oscillations; only fires when price breaks out of the channel   |
//+------------------------------------------------------------------+
bool ComputeChannelBandSL(bool isBuy, double &slLevel)
{
    ...
    if(stdDev < _Point) return false; // degenerate case

    slLevel = isBuy ? (lineVal - InpChannelSL_StdDevMul * stdDev)
                    : (lineVal + InpChannelSL_StdDevMul * stdDev);
    return true;
}

//+------------------------------------------------------------------+
//| Slope alignment cycle: divergence → alignment → entry           |
//+------------------------------------------------------------------+
void EvaluateSlopeAlignmentCycle(bool &bullishEntry, bool &bearishEntry)
{
    ...
    if(InpVerboseEntryLogs)
        PrintFormat("Slope cycle: priceSlope=%.8f rsiSlope=%.8f | sawBullDiv=%s sawBearDiv=%s",
                    ps, rs, g_saw_bullish_div ? "yes" : "no", g_saw_bearish_div ? "yes" : "no");

    // Step 1: Register divergence using stable long-window slopes
    double rsi1 = GetRSI(1);
    if(bullish) {
        if(!g_saw_bullish_div)
            PrintFormat("[DIV] BULLISH divergence | price slope: %+.6f (dn) | RSI slope: %+.6f (up) | RSI: %.1f | lookback: %d bars",
                        ps, rs, rsi1, GetSlopePeriod());
        g_saw_bullish_div = true;  g_saw_bearish_div = false;
    }
    if(bearish) {
        if(!g_saw_bearish_div)
            PrintFormat("[DIV] BEARISH divergence | price slope: %+.6f (up) | RSI slope: %+.6f (dn) | RSI: %.1f | lookback: %d bars",
                        ps, rs, rsi1, GetSlopePeriod());
        g_saw_bearish_div = true;  g_saw_bullish_div = false;
    }

    // Step 2: Entry alignment uses shorter responsive window
    int entryPeriod = (InpEntry_Lookback_Bars > 2) ? InpEntry_Lookback_Bars : GetSlopePeriod();
    bool eDummy1, eDummy2;
    double eps, ers;
    if(!EvaluateSlopeDivergence(eDummy1, eDummy2, eps, ers, entryPeriod)) return;

    if(InpVerboseEntryLogs)
        PrintFormat("[WAIT] Pending alignment | div_slope(p=%d): ps=%+.6f rs=%+.6f | entry_slope(p=%d): ps=%+.6f rs=%+.6f | bullDiv=%s bearDiv=%s",
                    GetSlopePeriod(), ps, rs, entryPeriod, eps, ers,
                    g_saw_bullish_div ? "Y" : "N", g_saw_bearish_div ? "Y" : "N");

    if(g_saw_bullish_div && eps < 0.0 && ers < 0.0)
    {
        bullishEntry = true;
        g_saw_bullish_div = false;
        PrintFormat("[ALIGN] BUY signal | div slope(p=%d): ps=%+.6f | entry slope(p=%d): ps=%+.6f rs=%+.6f | RSI: %.1f",
                    GetSlopePeriod(), ps, entryPeriod, eps, ers, rsi1);
    }
    if(g_saw_bearish_div && eps > 0.0 && ers > 0.0)
    {
        bearishEntry = true;
        g_saw_bearish_div = false;
        PrintFormat("[ALIGN] SELL signal | div slope(p=%d): ps=%+.6f | entry slope(p=%d): ps=%+.6f rs=%+.6f | RSI: %.1f",
                    GetSlopePeriod(), ps, entryPeriod, eps, ers, rsi1);
    }
}
...
void CheckForEntrySignals()
{
    ...
    if(bullishSignal)
    {
        ...
        PrintFormat("[ENTRY] BUY @ %.5f | lots: %.2f | SL: %.5f (%s, %.0f pts) | TP: %.5f",
                    ...
                    tp);
        if(!trade.Buy(lots, _Symbol, price, sl, tp, "TRE_Buy_v24"))
    ...
    else
    {
        ...
        PrintFormat("[ENTRY] SELL @ %.5f | lots: %.2f | SL: %.5f (%s, %.0f pts) | TP: %.5f",
                    ...
                    gotBand ? "channel" : "fixed", slPts,
                    tp);
        if(!trade.Sell(lots, _Symbol, price, sl, tp, "TRE_Sell_v24"))
...
void ManageOpenTrades()
{
    ...
    // Exit when price slope (entry window) reverses sign — same window used for entry trigger
    if(InpUse_Slope_Alignment_Exit)
    {
        ...
        bool exitSell = !isBuy && ps < 0.0;

        if(exitBuy || exitSell)
        {
            PrintFormat("[EXIT] %s closed -price slope reversed (%+.6f, p=%d) | profit: %+.0f pts | open: %.5f close: %.5f",
                        isBuy ? "BUY" : "SELL", ps, exitPeriod, profitPts, openPrice, currentPrice);
